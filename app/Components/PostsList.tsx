...
export default function PostsList({ posts, maxDisplay }: PostsType) {
    return (
        <div className="flex flex-none flex-col initial:mb-[-1px] sm:mb-0 initial:gap-7 sm:gap-3 border-solid border-l-[1px] border-gray-200 dark:border-zinc-700 h-min w-full box-border content-start items-start justify-start relative">
            {posts.slice(0, maxDisplay).map((post) => (
                <div key={post.path} className="flex flex-none initial:flex-col sm:flex-row initial:gap-0 sm:gap-10 w-full content-start items-start justify-start h-min box-border static">
                    <header className="flex flex-col grow shrink-0 basis-0 h-min initial:pb-2 sm:pb-3 box-border content-start items-start justify-start max-w-md initial:static sm:sticky top-20 z-10">
                        ...
                        <div className="flex flex-col shrink-0 box-border justify-start flex-none h-auto whitespace-pre-wrap w-full break-words outline-none relative">
                            <h2 className="whitespace-pre-wrap box-border tracking-tight text-start">
                                <Link className="ml-5 text-3xl font-semibold hover:underline hover:decoration-solid hover:decoration-orangeSite hover:underline-offset-4" href={`/${post.path}`}>
                                    {post.title}
                                </Link>
                            </h2>
                        </div>
                    </header>
                    <div className="initial:ml-5 sm:ml-0 flex flex-col grow shrink-0 basis-0 gap-4 box-border content-start items-start justify-start h-min relative">
